...
export default function Homefrançais() {
  ...
  const goToTest = (classe: string, serie?: string) => {
    ...
    const target = `/français/test/questions/${niveauToUrlPart(classe)}/${serieSlug}`;
    navigate(target);
  };
  ...
  const handleSeriesSelect = (serie: Serie) => {
    ...
    if (!hasSubSeries && selectedLevel) {
      setConfirmation(`Tu as choisi : ${selectedLevel} ${serie}`);
  ...
  const handleSubSeriesSelect = (subSerie: string) => {
    if (selectedLevel) {
      setConfirmation(`Tu as choisi : ${selectedLevel} ${subSerie}`);
    ...
    >
      <div className="min-h-screen flex flex-col items-center justify-center px-6 py-8 text-white z-20">
        ...
        <p className="mb-6 text-lg text-white/90">
          Choisissez votre classe pour commencer le test de la langue FRANÇAISE :
        </p>

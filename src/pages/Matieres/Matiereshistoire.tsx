// 📁 Matiere.tsx (Histoire Harmonisé)
import React from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import { useAuth } from "../../hooks/useAuth";
import { FaFlag, FaLandmark, FaGlobeAmericas, FaGlobeEurope, FaGlobeAsia, FaGlobeAfrica } from "react-icons/fa";
import { GiEgyptianProfile, GiAncientColumns } from "react-icons/gi";

const Matiere: React.FC = () => {
  const { loading } = useAuth();
  const navigate = useNavigate();

  const histoireOptions = [
    { name: "benin", label: "Bénin", icon: GiAncientColumns, color: "text-yellow-600" },
    { name: "france", label: "France", icon: FaLandmark, color: "text-blue-600" },
    { name: "etatsunis", label: "États-Unis", icon: FaGlobeAmericas, color: "text-red-600" },
    ...
    { name: "inde", label: "Inde", icon: FaLandmark, color: "text-orange-600" },
    { name: "urss", label: "URSS", icon: FaGlobeEurope, color: "text-red-800" },
    { name: "afrique", label: "Afrique", icon: FaGlobeAfrica, color: "text-green-600" },
    { name: "amerique", label: "Amérique", icon: FaGlobeAmericas, color: "text-blue-600" },
    { name: "asie", label: "Asie", icon: FaGlobeAsia, color: "text-yellow-500" },
  ];
  ...
  const handleChoice = (pays: string) => {
    navigate(`/home/homehistoire${pays}`);
  };
